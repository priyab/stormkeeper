...
(function() {
  ...
  this.include = function() {
    ...
    this.post({
      '/tokens': function() {
        return this.send(agent.authorize(new StormKeeper.prototype.StormToken(null, this.body)));
      }
    });
    this.get({
      '/tokens/:id': function() {
    ...
    this.del({
      '/tokens/:id': function() {
        ...
        match = agent.tokens.get(this.params.id);
        if (match != null) {
          return this.send(agent.revoke(match));
        } else {
          return this.send(404);
        }
      }
    ...
    this.post({
      '/rules': function() {
        return this.send(agent.authorize(new StormKeeper.prototype.StormRule(null, this.body)));
      }
    });
    this.get({
      '/rules/:id': function() {
